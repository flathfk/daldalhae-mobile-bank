...
function txLabel(type) {
  return { DEPOSIT: "충전", WITHDRAW: "결제", TRANSFER_OUT: "선물", TRANSFER_IN: "선물 받음" }[type] || type;
}
    ...
    return (
      <main className="auth-shell">
        <section className="auth-card">
          ...
          <p className="auth-copy">회원, 지갑, 거래, Redis 감사 로그를 관리하는 PC 전용 운영 화면입니다.</p>
    ...
    <div className="admin-layout">
      <aside className="sidebar">
        ...
        <nav className="side-nav">
          <NavItem active icon="🏠" label="대시보드" />
          <NavItem icon="👤" label="회원 관리" />
          <NavItem icon="👛" label="지갑 관리" />
          <NavItem icon="📊" label="거래 모니터링" />
          <NavItem icon="📝" label="Redis 감사 로그" />
        </nav>
        ...
        <main className="content">
          {dashboard.isError && (
            <section className="panel danger-panel">
              <div className="panel-head"><h2>세션 만료</h2><span>다시 로그인 필요</span></div>
              <p className="error-text">관리자 인증이 만료되어 데이터를 불러오지 못했습니다. 다시 로그인해주세요.</p>
              <button className="secondary" onClick={auth.logout}>다시 로그인</button>
            </section>
          )}
          ...
          <section className="metric-grid">
            ...
            <div className="metric-card"><span>총 충전금</span><strong>{formatMoney(totalBalance)}</strong><p>대시보드 캐시 대상</p></div>
          </section>
          ...
          <section className="operation-grid">
            ...
            <div className="panel">
              ...
              <input placeholder="초기 충전금" value={account.initialBalance} onChange={(e) => setAccount({ ...account, initialBalance: Number(e.target.value) })} />
          ...
          <section className="data-grid">
            ...
            <div className="table-panel wide">
              ...
              <div className="table-wrap">
                <table><thead><tr><th>ID</th><th>User</th><th>지갑번호</th><th>충전금</th><th>상태</th></tr></thead><tbody>{accounts.map((a) => <tr key={a.id}><td>{a.id}</td><td>{a.userId}</td><td>{a.accountNumber}</td><td>{formatMoney(a.balance)}</td><td><span className={`status ${String(a.status).toLowerCase()}`}>{String(a.status)}</span></td></tr>)}</tbody></table>
              </div>
            </div>
