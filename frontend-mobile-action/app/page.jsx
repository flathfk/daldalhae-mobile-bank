...
export default function MobileActionPage() {
  ...
  const [memo, setMemo] = useState("기프티콘 선물");
      ...
      <main className="mobile-shell action-shell">
        <section className="device-card login-card">
          ...
          <section className="signin-hero">
            ...
            <h2>충전·결제·선물은<br />로그인 후 진행합니다</h2>
            <p>
              조회 화면에서 로그인한 인증과 같은 Redis 저장소를 사용합니다.
  ...
  return (
    <main className="mobile-shell action-shell">
      <section className="device-card">
        ...
        {account.isError && (
          <section className="form-card">
            <p className="error-text">로그인이 만료되어 데이터를 불러오지 못했습니다. 다시 로그인해주세요.</p>
            <button className="secondary-btn full" onClick={auth.logout}>다시 로그인</button>
          </section>
        )}
        ...
        <section className="account-card google-blue-card">
          ...
          <h2>{account.data ? money(account.data.balance) : "충전금 조회 중"}</h2>
        ...
        <section className="form-card transfer-card">
          ...
          <div className="action-buttons">
            ...
            <button className="secondary-btn" disabled={isBusy} onClick={() => withdraw.mutate({ amount: Number(amount), memo: memo || "결제" })}>결제</button>
            <button className="primary-btn span2" disabled={isBusy} onClick={() => transfer.mutate({ toAccountNumber: to, amount: Number(amount), memo: memo || "선물" })}>선물 보내기</button>
            <button className="dark-btn span2" disabled={isBusy} onClick={() => multi.mutate({ memo: "여러 명 선물", targets: [{ toAccountNumber: "110-100-000002", amount: 1000 }, { toAccountNumber: "110-100-000003", amount: 2000 }] })}>여러 명에게 선물</button>
          </div>
        ...
        {result && (
          <section className="result-card">
            ...
            <p>현재 충전금 {money(result.account?.balance)}</p>
          </section>
        )}
            ...
            {(tx.data || []).slice(0, 6).map((item) => (
              <div className="tx-item" key={item.id}>
                <div>
                  ...
                  <strong>{item.memo || "활동"}</strong>
                  <p>{item.createdAt}</p>
                </div>
                <b>{money(item.amount)}</b>
              </div>
            ))}
            {!(tx.data || []).length && <p className="empty">아직 활동 내역이 없습니다.</p>}
          </div>
        </section>

        <nav className="bottom-nav">
          <a href="/">홈</a>
          <span className="active">결제</span>
          <a href="/">내역</a>
