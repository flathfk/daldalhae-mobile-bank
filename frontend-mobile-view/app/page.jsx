...
export default function MobileViewPage() {
  const [form, setForm] = useState({ username: "user1", password: "1234", name: "카페회원" });
    ...
    return (
      <main className="phone-bg">
        <section className="phone-card auth-card">
          ...
          <p className="eyebrow">DALDALHAE</p>
          <h1>달달한 하루<br />모바일 카페 지갑</h1>
          <p className="lead">조회는 모바일 조회 화면, 충전·결제는 별도 결제 화면에서 처리됩니다. 로그인 세션은 Redis에 저장됩니다.</p>
    ...
    <main className="phone-bg">
      <section className="phone-frame">
        ...
        <header className="mobile-header">
          ...
          <button className="icon-button" onClick={auth.logout}>로그아웃</button>
        </header>

        {account.isError && (
          <section className="notice-card">
            <div><strong>로그인이 만료되었습니다</strong><p>데이터를 불러오지 못했습니다. 다시 로그인해주세요.</p></div>
            <button className="icon-button" onClick={auth.logout}>다시 로그인</button>
          </section>
        )}
        ...
        <section className="notice-card">
          <div><strong>Redis 세션 공유</strong><p>조회 화면 3002에서 로그인해도 결제 화면 3003에서 같은 인증 상태를 사용합니다.</p></div>
          <span>LIVE</span>
        </section>
        ...
        <section className="section-block">
          <div className="section-head"><h2>충전·결제 내역</h2><button onClick={() => { account.refetch(); tx.refetch(); recent.refetch(); }}>새로고침</button></div>
          {latest && <div className="latest-card"><span>최근 활동</span><strong>{txLabel(latest.type)}</strong><b>{money(latest.amount)}</b><p>{latest.memo || "메모 없음"}</p></div>}
          <div className="tx-list">
            ...
            {!transactions.length && <p className="empty">아직 활동 내역이 없습니다.</p>}
          </div>
        </section>
